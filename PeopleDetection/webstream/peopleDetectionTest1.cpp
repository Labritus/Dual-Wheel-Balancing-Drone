...
const int HTTP_PORT = 8080; // Port for HTTP server
std::mutex frame_mutex;     // Mutex for frame access
cv::Mat latest_frame;       // Latest processed frame for streaming
std::condition_variable frame_cv; // Condition variable for frame updates
std::atomic<bool> new_frame_available{false}; // Flag to indicate new frame is available
...
void streamFrameHandler(int client_socket);
...
private:
    ...
    std::thread mStreamThread; // Thread for HTTP streaming
...
vector<String> getOutputsNames(const Net& net)
{
    ...
    return names;
}Failed to start camera manager: " << ret << endl;
            return false;
        }
        ...
        try {
            ...
            try {
                ...
                if (frame.empty() || frame.rows <= 0 || frame.cols <= 0) {
                    ...
                    // Make a copy of the frame for streaming
                    {
                    ...
                    // Display image (if running locally with GUI)
...
public:
    ...
    bool initialize() {
        ...
        // Start HTTP streaming server in a separate thread
        mStreamThread = std::thread(httpServerThread);
...
// HTTP Server handler to stream video frames
void httpServerThread() {
    ...
    if (bind(server_fd, (struct sockaddr *)&address, sizeof(address)) < 0) {
        cerr << "Failed to bind to port " << HTTP_PORT << endl;
        return;
    }
    ...
    cout << "HTTP server started on port " << HTTP_PORT << endl;
    ...
    while (g_running) {
        ...
        std::thread client_thread(streamFrameHandler, new_socket);
...
// Handle an individual client connection for streaming
void streamFrameHandler(int client_socket) {
    try {
        // Check the HTTP request (simplified)
        char buffer[1024] = {0};
        int valread = read(client_socket, buffer, 1024);
        string request(buffer);
        
        if (request.find("GET /video") != string::npos) {
            ...
            while (g_running) {
                cv::Mat frame_copy;
                {
                    ...
                    if (!latest_frame.empty()) {
                        ...
                        cv::resize(latest_frame, frame_copy, cv::Size(640, 480));
                ...
                if (!frame_copy.empty()) {
                    ...
                    cv::imencode(".jpg", frame_copy, jpeg_buffer, params);
                    
                    // Send frame header
                    stringstream frame_header;
                    frame_header << "--frame\r\n";
                    frame_header << "Content-Type: image/jpeg\r\n";
                    frame_header << "Content-Length: " << jpeg_buffer.size() << "\r\n\r\n";
                    string frame_header_str = frame_header.str();
                    send(client_socket, frame_header_str.c_str(), frame_header_str.length(), 0);
                    
                    // Send frame data
                    send(client_socket, jpeg_buffer.data(), jpeg_buffer.size(), 0);
                    
                    // End frame
                    string frame_end = "\r\n";
                    send(client_socket, frame_end.c_str(), frame_end.length(), 0);
                }
            ...
            // Serve a simple HTML page to redirect to the main interface
            string html = "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\n\r\n";
            html += "<!DOCTYPE html><html><head>";
            html += "<meta http-equiv=\"refresh\" content=\"0; URL='/index.html'\" />";
            html += "</head><body><p>Redirecting to control page...</p></body></html>";
            send(client_socket, html.c_str(), html.length(), 0);
        } else if (request.find("GET /index.html") != string::npos) {
            // Redirect to control page (hosted elsewhere)
            string response = "HTTP/1.1 302 Found\r\n";
            response += "Location: http://" + string(inet_ntoa(address.sin_addr)) + ":80/index.html\r\n\r\n";
        ...
        cerr << "Exception in stream handler: " << e.what() << endl;
    }
