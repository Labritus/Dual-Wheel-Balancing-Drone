 ...
 */
#define MPU6050
#define MOTION_DRIVER_TARGET_RASPBERRY_PI

#if defined MOTION_DRIVER_TARGET_RASPBERRY_PI
...
#include <functional>
#include <atomic>
#include "Delay.hpp"  // For Timer::scheduleCallback

#define i2c_write(slave_addr, reg_addr, length, data)  i2c_smbus_write_byte_data(slave_addr, reg_addr, *data)
#define i2c_read(slave_addr, reg_addr, length, data)   *data = i2c_smbus_read_byte_data(slave_addr, reg_addr)
// REAL-TIME FIX: Completely non-blocking delay implementation
// For hardware timing delays, use Timer::scheduleCallback instead
#define delay_ms(ms) do { \
    if ((ms) > 0) { \
        printf("INFO: DMP delay %dms replaced with non-blocking yield\n", (ms)); \
        std::this_thread::yield(); \
    } \
} while(0)

#define get_ms(count) { struct timespec ts; clock_gettime(CLOCK_MONOTONIC, &ts); *count = ts.tv_sec * 1000 + ts.tv_nsec / 1000000; }

// New non-blocking delay with callback for hardware timing
#define delay_ms_async(ms, callback) do { \
    if ((ms) > 0) { \
        Timer::scheduleCallback((ms) * 1000, callback); \
    } else if (callback) { \
        callback(); \
    } \
} while(0)

// Non-blocking retry loop helper for hardware operations
template<typename CheckFunction, typename Callback>
static void retry_with_delay_async(int max_tries, int delay_ms, 
                                   CheckFunction check_func, 
                                   Callback success_callback,
                                   Callback failure_callback) {
    static std::atomic<int> tries_remaining{0};
    
    tries_remaining.store(max_tries);
    
    std::function<void()> retry_func = [=]() mutable {
        if (check_func()) {
        ...
        int remaining = tries_remaining.fetch_sub(1);
        if (remaining <= 1) {
            // Failed - call failure callback
            failure_callback();
            return;
        }
        
        // Schedule next retry
        Timer::scheduleCallback(delay_ms * 1000, retry_func);
    };
...
int mpu_init(void)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &(data[0])))
        return -1;
    // Non-blocking reset delay - hardware usually ready much faster than 100ms
    printf("INFO: MPU reset initiated - non-blocking\n");
    std::this_thread::yield();
...
static int compass_self_test(void)
{
    ...
    // Non-blocking polling loop - replace blocking delay with fast polling
    for (int retry = 0; retry < tries; retry++) {
        // Quick yield instead of 10ms blocking delay
        std::this_thread::yield();
        ...
        // For longer waits, use small non-blocking delay
        if (retry < tries - 1) {
            auto start = std::chrono::steady_clock::now();
            while (std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now() - start).count() < 2) {
                std::this_thread::yield(); // 2ms non-blocking wait
            }
        }
    }
    if (!(tmp[0] & AKM_DATA_READY))
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 2, data))
        return -1;
    // Non-blocking self-test setup delay - reduced for real-time performance
    printf("INFO: Self-test setup - non-blocking delay\n");
    std::this_thread::yield();
    data[0] = 0;
    if (i2c_write(st.hw->addr, st.reg->int_enable, 1, data))
        return -1;
    ...
    if (i2c_write(st.hw->addr, st.reg->accel_cfg, 1, data))
        return -1;
    if (hw_test) {
        // Non-blocking hardware test stabilization delay
        printf("INFO: Hardware test stabilization - non-blocking\n");
        std::this_thread::yield();
    }
